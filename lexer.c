...
/*
    `get_tok` is a state machine. It takes a newline terminated string
    `buffer` and the length of this string. It parses this string into a list
    of tokens. The list of possible types of tokens in the token list can be
    found in `token.h`.
    `get_tok` is initialized in the UNKNOWN state. It will then iterate through
    the buffer and enter one of the following states:
    ...
    * SYMBOL if the current character is anything else, but
    ...
    encountered and `get_tok` will immediately stop lexing and return the token
    list.
    ...
    * SYMBOL will exit if the current character is alphnumeric, `_`, or `\n`.
    ...
    Pre Conditions:
    * `buffer` is NULL terminated and as long as `buffer_len`.
    Return value:
...
/*
    Allocate a new token of type `tok_identifier`, `tok_extern`, or `tok_def`.
    ...
    be NULL.
    Pre Conditions:
    * `token_begin` and `tok_end` are not NULL
    * `token_end` is greater than `token_end`.
    Return value:
    A newly allocated token. If the new token is of type `tok_identifier` its
    value will be another new allocation.
*/
